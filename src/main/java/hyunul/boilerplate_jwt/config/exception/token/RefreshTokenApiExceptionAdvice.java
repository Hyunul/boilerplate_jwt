package hyunul.boilerplate_jwt.config.exception.token;

import hyunul.boilerplate_jwt.api.token.exception.RefreshTokenException;
import hyunul.boilerplate_jwt.config.exception.common.ApiExceptionEntity;
...
public class RefreshTokenApiExceptionAdvice {
    ...
    public ResponseEntity<ApiExceptionEntity> exceptionHandler(HttpServletRequest req, final RefreshTokenException e) {
        log.warn("[RefreshTokenApiExceptionAdvice] RefreshTokenException :: {}",
                e.getRefreshTokenExceptionResult().getMessage());
